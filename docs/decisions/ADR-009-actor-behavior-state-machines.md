...
- **Damage-driven conditions** (`damaged`, `health_below`, and `allies_within`) and **attack actions** (`melee`, `fire`, `spit`, `detonate`, and contact damage on `charge`) have joined the sets, each with its numbers on the state (a `BehaviorAttack`) and its defaults per action. An attack appends to an effects buffer (`src/game/combat`) that the weapon-fire, projectile and damage phases carry out, never writing state directly ([project-format.md §9](../editor/project-format.md#9-what-it-becomes)) — so the sets stayed closed and the format only grew keys.
...
- **Actors targeting actors** — a guard fighting a raider — arrived with the neighbour grid, as a sense rather than an action: `targets` is `players` (the default, unchanged) or `opponents`. The action sets stayed closed; only whom `pursue` pursues widened.
- **Flow fields** replaced per-actor A* for pursuing a player it perceives, behind the same `pursue` action; nothing in the behavior format changed.
