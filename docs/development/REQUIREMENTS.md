...
| `SimplishDependencies` | `FetchContent` declarations with pinned versions. Deliberately few: nlohmann_json, FreeType, stb (images, and stb_vorbis for audio), Catch2, and SDL3 on desktop (windowing, pads, and audio output). Audio added no dependency ([ADR-010](../decisions/ADR-010-software-mixer.md)); networking's are added when it is written |
