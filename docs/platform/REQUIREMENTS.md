...
| Vulkan backend | `src/platform/render/backends/vulkan/` | 29 | Device, command list, handle table, format map; the built-in GUI, static mesh, skinned mesh, outline, effects, volumetric-smoke and water pipelines compiled from GLSL by glslang; one shared push-descriptor layout for stage bytes and four fragment textures (slot 0 at binding 5, slots 1–3 after the samplers); `copyTexture` by `vkCmdCopyImage`, leaving the copy readable; per-frame upload ring; tracked image layouts; deferred destruction. Runs on MoltenVK with its GPU tests passing under validation — see [Development §3.2](../development/REQUIREMENTS.md#32-running-vulkan-on-macos) |
| DX12 backend | `src/platform/render/backends/dx12/` | 25 | Device, command list, handle table, descriptor heap allocator, frames-in-flight, resource wrappers, shared root signature (a descriptor table per fragment texture slot, `t0`–`t3`), `copyTexture` by `CopyTextureRegion`, built-in GUI, static mesh, skinned mesh, outline, effects, volumetric-smoke and water pipelines |
...
| UDP transport | `src/platform/net/` | 6 | `listenUdp`, `connectUdp`: the engine's `NetTransport` over ENet 1.3.18 — one reliable ordered channel, peers dropped after 2–8 s of silence, messages capped at 64 KB. `UdpLanBeacon`, `findLanGames`: finding sessions on the LAN. Desktop only; a console session goes through its distributor's relay ([networking.md](../engine/networking.md), [ADR-013](../decisions/ADR-013-server-relayed-lockstep.md)) |
...
| PLT-RHI-8 | A draw may bind `RHI_MAX_FRAGMENT_TEXTURES` (4) fragment textures on every backend, and `copyTexture` copies one texture — the backbuffer included — into another of `backbufferFormat()` outside a pass, so a draw over the scene can see through it ([water.md §4](../engine/water.md#4-drawing)). Metal's drawables are made copyable (`framebufferOnly = NO`); Vulkan's swapchain images are made with `TRANSFER_SRC` where the surface allows it, and a copy is left in `SHADER_READ_ONLY` for the pass after; OpenGL, which draws every pass into the default framebuffer, copies that with `glCopyTexSubImage2D`, in its own bottom-up rows |
