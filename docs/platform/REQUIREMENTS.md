...
| UDP transport | `src/platform/net/` | 3 | `listenUdp`, `connectUdp`: the engine's `NetTransport` over ENet 1.3.18 — one reliable ordered channel, peers dropped after 2–8 s of silence. Desktop only; a console session goes through its distributor's relay ([networking.md](../engine/networking.md), [ADR-013](../decisions/ADR-013-server-relayed-lockstep.md)) |
