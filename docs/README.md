...
| [Water](engine/water.md) | Working on painted water: the ripple simulation, water depth and flow, the water surface, its shader, how it is lit, refracts and reflects, the scene copy it is drawn from, wakes and splashes, or the water fidelity setting |
...
| [Game SDK](game/sdk.md) | Writing a project's own game in C++: the `Game` base and its event hooks, entity queries and per-entity data, timers, spawn patterns |
| [Game screens](game/ui.md) | A game's own menus and HUD: `content/ui/*.ui.json`, built from the GUI widgets; showing them from the logic, a choice as input, and the agent tools to write, render and press them |
