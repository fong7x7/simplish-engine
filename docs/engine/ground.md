...
- **Terrains.** `EDITOR_TERRAINS` (`editor-terrains.h`) is the built-in palette, in stacking order: grass, dirt, sand, stone, road, hole. Water is not a terrain: it lies over the ground on a layer of its own. Each has a name, the word a file and the agent API use for it, a colour and a grain. `makeEditorGroundAtlas` (`editor-ground-atlas.h`) turns them into the atlas: each swatch its colour, speckled by a hash of the texel's place, so it is the same on every run and tiles by construction.
...
- **Selecting an area.** With the Select tool, a click on painted ground — where no prop is — selects every cell of that terrain joined to the one clicked, corner to corner included, as it is drawn (`connectedGroundCells`, `ground-region.h`). The viewport outlines the area's edge, and the properties panel names its terrain and size with a **Terrain** row: picking another repaints every cell of it as one undoable edit, keeping the same cells selected, and Erase or the Delete key paints it bare. The cells are kept rather than found again, so repainting sand as the grass beside it does not swallow the grass into the selection.
