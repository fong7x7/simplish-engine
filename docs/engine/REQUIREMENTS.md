...
**Transparent effects, as built** ([fx.md](fx.md)): particles draw straight after the outline, in the same pass with no depth attachment, and read the scene's depth as a texture — hidden behind geometry, and faded over the last stretch in front of it, without writing or testing depth. They are laid out as camera-facing quads on the CPU, sorted back to front, and drawn in one call through a backend builtin, `RhiDevice::tryCreateFxParticlePipeline`, with premultiplied blending, so additive glows and smoke that hides what is behind it share the draw. A particle may turn on its own spin, be broken up into a puff of procedural noise rather than a smooth disc, and take its colour from the scene's lights instead of its own.

**Volumetric smoke, as built** ([fx.md §5](fx.md#5-volumetric-smoke)): a second builtin, `RhiDevice::tryCreateFxVolumePipeline`, draws in the same pass against the same depth, just before the particles. Each cloud (`FxVolume`) is a box of procedural noise whose fragments march a ray from the front of the box until the scene's depth says a surface is there, so smoke fills a doorway and wraps a crate rather than cutting against them. The camera being orthographic and fixed is what makes it cheap: the view ray is the matrix's depth row, and the CPU hands the fragment stage its ray by interpolating the corners of the rectangle the box covers, with no matrix in the shader.
...
Built so far: muzzle flashes, hits and blasts already light meshes, as short-lived point lights (`FxFlash`) handed to the mesh shader beside the level's placed lights. That shader takes `MESH_MAX_LIGHTS` (eight) lights a draw, placed lights first and the brightest flashes in whatever slots are left ([fx.md §6](fx.md#6-flashes)); the clustered path is what lifts that to 256.
...
| Painted ground (terrain grid, autotiling, tile layer) | [ground.md](ground.md) | First slice built: `engine/render-ground` holds a grid of terrain numbers, shapes each quarter-cell from its neighbours so painted areas join with rounded edges and filled bends, stacks the terrains as flat layers in one mesh, and codes the grid as runs for the level file. The editor's Tile tool paints it. Not read by the simulation yet |
| Water (a layer over the ground, ripple simulation, translucency, fidelity) | [water.md](water.md) | First slice built: `engine/render-water` holds water as a layer of its own over the ground — each cell a depth, a colour and an opacity, blended between tiles — and a flow; simulates ripples on it with the shallow-water wave equation on the frame's clock, going round what stands in it, carried downstream by its flow, with foam that lingers; and draws it through a builtin, `tryCreateWaterPipeline`, in the pass after the scene's, from a copy of the scene and its depth (`RhiCommandList::copyTexture`): the ground seen through it bent by its ripples and faded red first and blue last into the water's colour, the sky and the scene mirrored in it, wet ground and lapping waves at its shores, foam rings where things stand in it, wind waves, caustics and foam, lit by the meshes' own lights. Splashes are thrown through the effects. Flat, Low or High, the user's setting. Metal, Vulkan, DX12 and OpenGL. Presentation only |
...
| Effects (GPU particles, decals, trails, screen shake) | [fx.md](fx.md) | M5 — first slice built: `engine/render-fx` throws bursts of particles, simulated on the CPU and drawn in one call over the scene's depth, raymarches clouds of volumetric smoke against that same depth, and flashes that light meshes; `game/fx` plays one on every shot fired, every shot that lands and every blast, from the combat cues each tick leaves, and a blast leaves a cloud standing. Metal, Vulkan, DX12 and OpenGL pipelines for both. Decals, trails, screen shake and the effect budget are not written |
...
| Audio (mixer, spatialisation, voice stealing, ducking, output) | [audio.md](audio.md) | M3 — first slice built: `engine/audio` decodes WAV and Ogg Vorbis, synthesises stand-in clips, and mixes 48 voices with priority stealing, buses, music ducking and equal-power panning around a listener, on the output's thread behind a lock-free queue; `platform/audio` opens an SDL3 stream on desktop; `game/fx` plays the four nearest of each kind of combat cue a tick, and the editor's playtest is heard. Recorded sounds as content, music streaming, volume settings, occlusion and reverb are not written ([ADR-010](../decisions/ADR-010-software-mixer.md)) |
...
| Networking (transport, lockstep session, input delay, desync detection) | [networking.md](networking.md) | **Built**, headless: server-relayed lockstep ([ADR-013](../decisions/ADR-013-server-relayed-lockstep.md)), loopback and ENet/UDP transports, drop to stand-in, desync halt; `simplish-game --serve`, `--host`, `--join`. No person at the controls of a networked run yet |
...
| **M3 — Content, Audio, UI** | JSON data tables with schema validation and hot-reload; the software mixer and a platform output, spatialisation, voice stealing, combat ducking; retained-mode GUI with layout, text, and theming; dev console, CVars, and the frame profiler |
