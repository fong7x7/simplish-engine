...
| Water (a layer over the ground, ripple simulation, translucency, fidelity) | [water.md](water.md) | First slice built: `engine/render-water` holds water as a layer of its own over the ground — each cell a depth, a colour and an opacity, blended between tiles — simulates ripples on it with the shallow-water wave equation on the frame's clock, and draws it through a fifth scene builtin, `tryCreateWaterPipeline`, premultiplied over the terrain under it: the water's colour covering as much of that as its depth and opacity say, with ripples, wind waves, sky, caustics and foam, lit by the meshes' own lights. Flat, Low or High, the user's setting. Metal, Vulkan, DX12 and OpenGL. Presentation only |
