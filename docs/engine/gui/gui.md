...
2. **Text shaping** — UTF-8 is set glyph by glyph with the font's `kern`-table kerning, in any size, weight, line height, letter spacing and tabular digits ([technical/text-pipeline.md §0](technical/text-pipeline.md#0-what-ships-read-this-first)). Ligatures, GPOS kerning, bidirectional and complex-script text need HarfBuzz, which is not linked.
...
- **Shapes, gradients and shadows** — built: per-corner radii, per-side borders, linear and radial gradients, and soft drop shadows are all drawn analytically in the GUI shader, with no extra passes. Nine-slice images and group transforms and opacity are built too ([technical/renderer.md §2](technical/renderer.md#2-vertex-format-and-shapes)). **Not built:** a frosted-glass background blur, which needs a copy of the scene behind the panel.
