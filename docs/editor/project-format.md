    ...
    "entities": [
      ...
      { "id": "emitter_01", "definition": "entity:fx_emitter",
        ...
        "properties": { "effect": "wall_sparks", "direction": [0.0, 0.0, 1.0],
          ...
          "spin": 0.0, "textured": 0.0, "lit": 0.0,
          "flash": 1.2, "flash_range": 1.8, "flash_time": 0.08 } },
      { "id": "sprite_01", "definition": "entity:sprite_billboard",
        "at": [7.5, 5.5, 0.0],
        "properties": { "sheet": "sprites/slime.png", "height": 1.0,
          "columns": 4, "rows": 1, "frames": 4, "fps": 12.0 } }
    ]
  }
}
```
...
**A prop may sound like its own surface, and an actor may name its feet.** `surface` is what a step on the prop sounds like in a playtest, in place of the painted ground under it — `wood` for a deck, `cloth` for a rug, `metal` for a grate; one of `ground`, `grass`, `dirt`, `sand`, `water`, `stone`, `wood`, `metal`, `cloth`. It is written only when the prop has one. `footsteps` is what an actor's feet sound like — `default`, `boots`, `bare`, `claws` or `heavy` — written only for a prop with a behavior whose feet are not the default. A word the editor does not know reads as no surface and as the default feet. The surface is presentation; the feet are too, but for their stride, which the simulation steps by when game logic listens for steps ([audio.md §9.1](../engine/audio.md#91-footsteps)).
...
"water": { "bounds": { "min_x": 3, "min_y": 3, "width": 3, "height": 1 },
           ...
           "opacity": [[110, 3]],
           "flow_heading": [[64, 3]], "flow_speed": [[0, 2], [170, 1]],
           "viscosity": [[0, 3]] }
```

`depth` is in `step`s of a tile — 0.0625, written so a reader never has to guess it — from 1 (a sixteenth) to 255 (almost sixteen tiles), and `0` is dry; `red`, `green` and `blue` are the water's sRGB colour and `opacity` how much of the ground a tile of it hides, 0 to 255; `flow_heading` is which way it flows, in 256ths of a turn anticlockwise from east, and `flow_speed` how fast, in 255ths of 1.5 tiles a second — 0 stands; `viscosity` how thick it is, from water at 0 to something like honey at 255 — all read only where there is water. A file without the flow or viscosity keys, written before water had them, reads as standing water. The layer is written only when there is water. A layer with another `step`, another `encoding`, or a run list that does not cover its `bounds` reads as no water at all.
...
| `footsteps` | What its feet sound like: `default`, `boots`, `bare`, `claws` or `heavy` ([audio.md §9.1](../engine/audio.md#91-footsteps)) | `default`; a word not in the list is too, and said |
...
| `footsteps` | What its feet sound like, as a character's are (§8.1) | `default` |
...
| Field | Meaning |
|---|---|
| `id` | The sound, as the audio bank names it: `combat.shot_fired`, `combat.shot_hit_body`, `combat.shot_hit_wall`, `combat.blast`; or a footstep, `step.<feet>.<surface>` — feet `default`, `boots`, `bare`, `claws` or `heavy`, surface `ground`, `grass`, `dirt`, `sand`, `water`, `stone`, `wood`, `metal` or `cloth`: `step.boots.sand`. A footstep the table does not name falls back to the nearest it does ([audio.md §9.1](../engine/audio.md#91-footsteps)) |
