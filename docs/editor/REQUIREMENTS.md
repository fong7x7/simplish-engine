...
- **A stand-in, not a character.** Until sprites exist a player is drawn as the built-in cylinder at a person's proportions, lit and shaded like everything else, with a column in player 1's colour over it. Nothing collides yet — `physics` and `spatial` are not written — so the player walks through props.

Every playtest records a replay and writes it to `data/playtests/<level>.replay` when it stops. Pause, single-step, speed, replay review, the multi-player preview and the debug overlays of §7 are not built; [capabilities.md §6.1](capabilities.md#61-playtest) is the register.
