...
| `src/editor/shell/` | Title bar, menu bar (File / Edit / Level / View / Help), tool toolbar (Select / Tile / Height / Prop / Entity), the viewport with left- or middle-drag pan, scroll zoom, and click-to-select, the asset strip that drags models, light sources and player starts into the world, the properties panel that edits whichever is selected, and the level file those placements are saved to and loaded from | Built; 565 tests |
...
- **A start is drawn whole, in its player's colour.** It has no mesh, so where a prop shows its footprint under geometry, a start shows a person-height column — green, orange, violet, magenta for players one to four — so the four read as four players at a glance, and the selection outline replaces the colour while one is selected.
...
- **A stand-in, not a character.** Until sprites exist a player is drawn as the built-in cylinder at a person's proportions, lit and shaded like everything else, with a column in player 1's colour over it. Props stop the player: each is a solid box in the playtest — the box the viewport outlines — and the player, an upright cylinder a little narrower than a tile, stops against it and slides along it (`engine/physics`). A prop's properties end in a **Collides** checkbox, ticked for every prop dropped, which a click anywhere on its row flips; one that is unticked is drawn with a faded footprint and walked through, for the grass, the rugs and the decals.
