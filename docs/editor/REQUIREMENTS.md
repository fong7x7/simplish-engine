...
Particle emitters are the fourteenth. General › effects holds a **Particle Emitter** card; one dropped in the level throws a burst of particles every interval and flashes a light over the meshes near it, live in the viewport while the level is edited and in a playtest ([fx.md](../engine/fx.md#8-in-the-editor)). Its properties panel opens on an **Effect** row that starts it from any burst the combat effects throw — muzzle flash and sparks, wall sparks, grit, hit spray, fireball, embers, smoke — or the smoke plume no combat effect throws, and lists every number of that burst below it: direction, interval, particle count, cone, speed, life, size, start and end colour and how much each hides, gravity, drag, streak, spin, whether it is textured as a puff of noise and whether the scene's lights colour it, and the flash's brightness, reach and length. The panel scrolls, a notch of the wheel a row, to hold them.

- **An emitter holds its burst, not a name for one.** `effect` says which preset it was last started from, and the Effect row marks it `(edited)` once any number has moved; changing a preset in `game/fx` never reaches into a level that already has its own numbers.
- **Emitters are presentation.** They are saved with the level so a level can carry its steam vents and fires, but no tick reads them, and a playtest plays them into its own effects as it plays combat's.

Sprite billboards are the fifteenth, and the first 2D art the editor places. General › sprites holds a **Sprite Billboard** card; one dropped stands feet-on-the-floor as an upright quad facing the camera, playing one frame of a sprite sheet at a time ([sprites.md](../engine/sprites.md)). Its properties panel opens on a **Sheet** row stepping through every `.png` and `.tga` under the project's assets — which the scan now lists beside the models — and below it rows for where it stands, how tall, and the grid its sheet is cut into and the speed it plays at. Three decisions here:

- **A billboard is drawn with the meshes, not over them.** It goes through the same depth-buffered scene pass, and the mesh fragment stage discards texels below `MESH_ALPHA_CUTOFF`, so its empty corners write nothing. A billboard behind a prop is hidden by it and one in front is not, per pixel and with no ordering rule — [ADR-003](../decisions/ADR-003-hybrid-iso-render-model.md)'s interleave, with the [2026-09-22 amendment](../decisions/ADR-003-hybrid-iso-render-model.md#amendment-2026-09-22-a-billboard-is-an-upright-quad-not-a-depth-ramp)'s upright quad in place of the per-pixel depth ramp it first called for.
...
The painted ground is the sixteenth, and the first thing the toolbar's **Tile** tool does. General › ground holds a card per terrain — grass, dirt, sand, stone, road, hole — an **Erase** card, and **Water** and **Dry** cards that lay water over the ground and take it off again — a selected body of water has Depth, colour, Opacity, Flow Direction, Flow Speed and Viscosity rows ([water.md](../engine/water.md)); dropping one on the viewport takes it as the brush, switches to the Tile tool and paints a dab there. With the Tile tool out, a left drag paints the cells under a square brush 1 to 9 tiles across — `[` and `]` resize it, the viewport outlines what it covers, the status line names it, and the middle button still pans. Painted areas are autotiled as they are drawn ([ground.md](../engine/ground.md)): a road rounds its own ends, bends with a curve and runs straight between, a patch of sand has soft corners, and a road laid through the sand sits on it. Saved in the level file's tile layer, and reachable through `paint_ground`, `get_ground` and `select`'s `ground` target. Four decisions:
...
- **An area is selected as it is drawn.** A click with the Select tool on painted ground selects every tile of that terrain joined to the one clicked — corner to corner included, because that is how the autotiling joins them — outlined in the viewport, with a Terrain row in the properties panel that repaints it all as one edit, and Delete to erase it. The cells are held, not found again after each change, so repainting an area in the terrain beside it does not grow the selection into its neighbour.
