...
Particle emitters are the fourteenth. General › effects holds a **Particle Emitter** card; one dropped in the level throws a burst of particles every interval and flashes a light over the meshes near it, live in the viewport while the level is edited and in a playtest ([fx.md](../engine/fx.md#7-in-the-editor)). Its properties panel opens on an **Effect** row that starts it from any burst the combat effects throw — muzzle flash and sparks, wall sparks, grit, hit spray, fireball, embers, smoke — and lists every number of that burst below it: direction, interval, particle count, cone, speed, life, size, start and end colour and how much each hides, gravity, drag, streak, and the flash's brightness, reach and length. The panel scrolls, a notch of the wheel a row, to hold them.
