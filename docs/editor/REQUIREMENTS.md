...
| `src/editor/shell/` | Title bar, menu bar (File / Edit / Level / View / Help), tool toolbar (Select / Tile / Height / Prop / Entity), the viewport with left- or middle-drag pan, scroll zoom, and click-to-select, the asset strip that drags models, light sources and player starts into the world, the properties panel that edits whichever is selected, and the level file those placements are saved to and loaded from | Built; 515 tests |
| `src/editor/agent/` | The agent API: 28 tools over the shell's own state, the JSON protocol, and the binding to a running editor | Built; 87 tests |
