...
| `get_water` | The named depths, the user's water fidelity and where it is kept, and what the level's water did last frame: whether a moving surface was drawn, how finely it is simulated, how much water there is, how much it is moving, and how often it has been pushed |
...
| `paint_water` | Lays water over a rectangle of the ground — at a `depth`, and where it was dry in a `color` (`#rrggbb`) and `opacity` — or, with `dry`, takes it off, as the Water and Dry cards do; the terrain under it is untouched |
