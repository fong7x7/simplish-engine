...
| `paint_water` | Lays water over a rectangle of the ground — at a `depth`, and where it was dry in a `color` (`#rrggbb`), `opacity`, `flow_direction` (degrees) and `flow_speed` (0 to 1) — or, with `dry`, takes it off, as the Water and Dry cards do; the terrain under it is untouched |
