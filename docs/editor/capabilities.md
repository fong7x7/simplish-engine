...
| Reduce the interface's motion | ✅ | `run_command` (`toggle_reduced_motion`), `get_state` (`reduce_motion`) | View › Reduce Motion: menus, dialogs, toasts, glides and state changes land at once ([motion.md](../engine/gui/technical/motion.md)). Saved to the user's graphics file |
| Inspect the editor's layout | ✅ | `get_widgets`, `run_command` (`toggle_layout_bounds`) | View › Show Layout Bounds draws every widget's box and the hovered one's margin, padding and content; `get_widgets` gives an agent the same tree with rects and flags ([inspecting.md](../engine/gui/technical/inspecting.md)) |
