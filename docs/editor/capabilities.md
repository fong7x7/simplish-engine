...
| Write game logic with an SDK | ✅ | — | `<game/sdk/sdk.h>`: a `Game` base with event hooks, entity queries, per-entity data, timers, spawn rings, dice ([sdk.md](../game/sdk.md)); an agent writes it in `src/` and builds it with `run_command` |
