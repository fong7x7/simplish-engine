...
namespace {
  ...
  /// reaches it.
  RhiTextureHandle createFieldTexture(RhiDevice& device, uint32_t width,
                                      uint32_t height,
...
bool WaterRenderer::sizeField(RhiDevice& device, uint32_t width,
                              uint32_t height) {
  if (texture_width_ == width && texture_height_ == height &&
      textures_[0] != RHI_TEXTURE_INVALID) {
    return true;
  }
  releaseField(device);
  // Still water, level and unsloped, over land.
  texels_.assign(static_cast<size_t>(width) * height * WATER_TEXEL_BYTES, 128);
  for (RhiTextureHandle& texture : textures_) {
    texture = createFieldTexture(device, width, height, texels_);
    if (texture == RHI_TEXTURE_INVALID) {
      releaseField(device);
      return false;
    }
