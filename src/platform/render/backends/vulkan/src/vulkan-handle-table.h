...
// DESIGN SUMMARY
// ============================================================================
// VulkanHandleTable: Generational handle table for Vulkan GPU resources.
//
// Responsibilities:
...
// - destroy() marks the slot as free and bumps the generation
...
constexpr uint64_t HANDLE_INDEX_MASK = 0xFFFFFFFF;

template <typename T> class VulkanHandleTable {
public:
...
private:
  ...
  static uint64_t encode(uint32_t generation, uint32_t index) {
    return (static_cast<uint64_t>(generation) << HANDLE_INDEX_BITS) |
  ...
  static Decoded decode(uint64_t handle) {
    auto index = static_cast<uint32_t>(handle & HANDLE_INDEX_MASK);
    auto generation = static_cast<uint32_t>(handle >> HANDLE_INDEX_BITS);
