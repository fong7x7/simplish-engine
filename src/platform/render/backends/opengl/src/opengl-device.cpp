...
RhiFormat OpenGlDevice::backbufferFormat() const {
  // The copy converts to whatever the texture holds; sRGB decodes it on
  // sampling as the other backends' copies are decoded.
...
namespace {
  ...
  /// How many `vec4`s the water's fragment block is — `WaterShading`: its
  /// matrix's four columns, then eleven more.
  constexpr int WATER_SHADING_VECTORS = 15;
...
void OpenGlDevice::executeCommand(const GlCmdCopyTexture& cmd) {
  // As `copyPassDepth` does for depth: the default framebuffer's colour,
  // in GL's bottom-up rows, which is how `gl_FragCoord` reads it back.
  auto it = textures_.find(cmd.dst);
  if (it == textures_.end()) {
    return;
  }
  const GlTextureEntry& tex = it->second;
...
void main() {
  ...
  /// and WATER_PS_HLSL_SOURCE in the DX12 one: the fragment stage's body is
  /// shared by all four word for word, below a prologue saying what its
  /// words mean in GLSL. The vertex stage reads `WaterVertexUniforms` as a
  /// matrix and a `vec4`, the fragment stage `WaterShading` as fifteen
  /// `vec4`s in the order they are laid out, the scene's lights as the mesh
  /// program does, and four textures at units 0 to 3.
...
in float in_opacity;
in vec3 in_color;
// `WaterShading` as 15 vec4s, in the order they are laid out.
uniform vec4 u_water[15];
...
layout(binding = 1) uniform sampler2D u_water_scene;
layout(binding = 2) uniform sampler2D u_water_depth;
layout(binding = 3) uniform sampler2D u_water_still;
out vec4 frag_color;

// The block, filled from `u_water` before anything reads it.
WaterShading s;

float4 water_field_at(float2 uv) { return texture(u_water_tex, uv); }

float4 water_still_at(float2 uv) { return texture(u_water_still, uv); }

float3 water_scene_at(float2 uv) { return texture(u_water_scene, uv).rgb; }

float water_scene_depth(int2 p) {
  int2 top = textureSize(u_water_depth, 0) - 1;
  return texelFetch(u_water_depth, clamp(p, int2(0), top), 0).r;
}

uint water_light_count() { return u_light_count; }

uint water_shade_bands() { return u_shade_bands; }

float4 water_light_register(uint i, int k) {
  return u_lights[3u * i + uint(k)];
}

// Where a point in clip space lands on the copy of the scene: GL's rows
// run up from the bottom, in the copy as in `gl_FragCoord`.
float2 water_screen_uv(float4 clip) {
  float2 ndc = clip.xy / clip.w;
  float2 pixel = s.screen.xy + float2(0.5f + 0.5f * ndc.x, 0.5f - 0.5f * ndc.y) *
                                   s.surface.xy;
  float2 uv = pixel * s.screen.zw;
  return float2(uv.x, 1.0f - uv.y);
}

// How deep a point in clip space lies, as the depth buffer holds it: GL
// maps clip depth from −1 to 1 into it, and the scene's runs from 0.
float water_clip_depth(float4 clip) { return 0.5f * clip.z / clip.w + 0.5f; }
...
void main() {
  s = WaterShading(mat4(u_water[0], u_water[1], u_water[2], u_water[3]),
                   u_water[4], u_water[5], u_water[6], u_water[7],
                   u_water[8], u_water[9], u_water[10], u_water[11],
                   u_water[12], u_water[13], u_water[14]);
  frag_color = water_shade(in_world, in_uv, in_depth, in_opacity, in_color,
                           gl_FragCoord);
}
)glsl";
...
namespace {
  ...
  /// vertex, drawn in the pass over the scene, which tests nothing — the
  /// water reads the scene's depth itself — and blended premultiplied.
