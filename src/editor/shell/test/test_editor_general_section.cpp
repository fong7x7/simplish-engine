...
/// assets, then the built-in shapes, then the lights.
...
TEST_CASE("the section holds two subsections and nothing itself") {
  const EditorAssetTree tree = treeWithSection(0);
  const size_t section = folderNamed(tree, EDITOR_GENERAL_FOLDER_NAME);

  // Two kinds of built-in thing in one grid of cards would read as a pile;
  // the subsections are what somebody reaching for a light navigates by.
  REQUIRE(tree.folders[section].assets.empty());
  REQUIRE(tree.folders[section].child_folders.size() == 2);
}
...
TEST_CASE("the lighting subsection holds every light") {
  ...
  REQUIRE(tree.folders[lighting].assets.size() == EDITOR_GENERAL_ITEM_COUNT);
