...
/// @file iso-camera.h
/// @brief Pan-and-zoom camera for the editor viewport.
...
///
/// Pan and zoom only — there is deliberately no free rotation. A project
/// picks one of two fixed projections and the renderer is allowed to depend
/// on that (docs/decisions/ADR-003-hybrid-iso-render-model.md); a camera
/// that could be turned to any angle would show the world at angles no
/// sprite is authored for.
...
struct IsoCamera {
  ...
  float zoom = 1.0f;
  /// How the world projects onto the screen, mirrored from the open
  /// project's setting. Carried here so that every `makeIsoView` call site
  /// gets it without passing it, and so a saved camera describes the view
  /// it was saved from.
  IsoAxes axes{};
};
...
inline void zoomCameraAt(IsoCamera& camera, const Rect& viewport, float notches,
  ...
  const IsoView before{viewport, camera.focus, camera.zoom, camera.axes};
  ...
  const IsoView after{viewport, camera.focus, camera.zoom, camera.axes};
...
[[nodiscard]] inline IsoView makeIsoView(const IsoCamera& camera,
                                         const Rect& viewport) {
  return {viewport, camera.focus, camera.zoom, camera.axes};
}
