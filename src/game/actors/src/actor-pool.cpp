...
ActorPool::ActorPool(uint32_t capacity)
  : slots(capacity), position(capacity), facing(capacity), home(capacity),
    ...
    attack_ready_tick(capacity), attack_lands_tick(capacity),
    death_blast_radius(capacity), death_blast_damage(capacity), state(capacity),
    state_since(capacity), target(capacity), target_kind(capacity),
