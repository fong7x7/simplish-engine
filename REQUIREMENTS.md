...
│   │   ├── spatial/            # Nav grid, line of sight, A*, flow fields, neighbour grid ✔ built
│   │   ├── render-iso/         # Isometric camera, projection, depth policy
│   │   ├── render-sprite/      # Sheet grids, billboard quads; atlas, batcher to come ✔ built (first slice)
│   │   ├── render-ground/      # Painted terrain grid, autotiled flat layers, RLE ✔ built (first slice)
...
│   │   ├── render-fx/          # Particles and flashes; decals, trails to come    ✔ built (first slice)
...
│   │   ├── build/              # Game logic builds, the library loader, deploys ✔ built
│   │   ├── shell/              # Title bar, menu bar, toolbar, viewport, assets ✔ built
│   │   ├── agent/              # The tool surface agents drive the editor with  ✔ built
│   │   └── deploy/             # The headless deployed game                     ✔ built
│   ├── game/                   # Gameplay — links engine
│   │   ├── content/            # Character definitions, the game's content     ✔ built
│   │   ├── player/             # Player pool, movement, health, downed and revive ✔ built
│   │   ├── combat/             # Effects buffer, projectiles, hazard pools, blasts ✔ built (first slice)
│   │   ├── actors/             # Enemies and NPCs: perception, behaviors, paths, attacks ✔ built
│   │   ├── world/              # The game's SimulationSystems, stand-in players ✔ built
│   │   ├── fx/                 # The effect each combat cue plays               ✔ built
│   │   ├── logic/              # The world a project's own C++ logic reaches    ✔ built
│   │   ├── sdk/                # The SDK a project's game logic is written with  ✔ built
│   │   ├── weapons/  director/  run/  coop/  — to write
│   └── bin/                    # Executables
...
|---|---|---|
| Engine | `math`, `core`, `image`, `render`, `gui`, `client`, `render-mesh`, `render-fx`, `render-sprite`, `render-ground`, `animation`, `gltf`, `sim`, `input`, `audio`, `physics`, `spatial` | Math (deterministic sine and cosine among it), allocators, logging, event bus, engine init, expression evaluator, plugin host, audit system, PCG32 and the fixed-step clock; the deterministic tick, SoA entity slots with generational handles, per-subsystem tick hashing, and replay record/encode/verify ([simulation.md](docs/engine/simulation.md)); input actions from keys and pads through a remappable binding scheme, quantised into the `PlayerInput` a tick runs on ([input.md](docs/engine/input.md)); an upright cylinder resolved out of axis-aligned boxes; a navigation grid built from those boxes, line of sight across it and deterministic A* over it ([spatial.md](docs/engine/spatial.md)); the abstract RHI interface (29 headers); a retained-mode GUI with layout, widgets, docking, theming, FreeType text, and a markdown renderer; `GameClient` / `RenderedGameClient`; rigged glTF models posed by animation clips, crossfaded between them, and skinned on the GPU, for the few characters that are not sprites ([animation.md](docs/engine/animation.md)); bursts of particles drawn over the scene's depth, and flashes that light meshes ([fx.md](docs/engine/fx.md)); sprite sheets cut into frames and the upright quads one frame is drawn on, cut out of their background by the mesh pass's alpha test ([sprites.md](docs/engine/sprites.md)); a painted grid of terrains, autotiled into flat stacked layers and run-length coded for the level file ([ground.md](docs/engine/ground.md)); WAV and Ogg clips, synthesised stand-ins, and a software mixer — voice stealing by priority, buses, music ducking, panning around a listener — on the output's thread ([audio.md](docs/engine/audio.md)) |
| Game | `content`, `player`, `combat`, `actors`, `logic`, `sdk`, `world`, `fx` | Players spawned from a `GameSetup` as the character each picked, taking its speed and health, moved by their stick on the deterministic tick, kept out of the level's solid props, and hashed. Actors — enemies and NPCs — that perceive the players, run a behavior's state machine ([ADR-009](docs/decisions/ADR-009-actor-behavior-state-machines.md)), plan paths round the props or walk a player's flow field, steer, turn, patrol routes laid out in the editor, take each other on, and attack — bites, volleys, hazard pools, blasts — through an effects buffer the damage phase applies, from twelve built-in behaviors or the project's own ([actors.md](docs/game/actors.md)) — 2,000 of them inside Engine §7's AI budget, measured by `scripts/perf-gate.sh`. Every shot fired, shot landed and blast is cued for presentation, and plays a muzzle flash, sparks or a spray, or a fireball, lighting the meshes around it ([fx.md](docs/engine/fx.md)), and is heard — the nearest four of each kind a tick, from where it happened ([audio.md](docs/engine/audio.md)). Enemy archetypes are data for the director to spawn. A project's own rules, in C++ in its `src/` and written with the game SDK ([sdk.md](docs/game/sdk.md)), run in the director's phase and are hashed with the rest — built by the editor into a library a playtest loads, and linked into `simplish-game` when the project is deployed ([logic.md](docs/game/logic.md), [ADR-011](docs/decisions/ADR-011-project-game-logic-in-cpp.md)). A determinism test and a replay round-trip over the whole world. No weapons, damage or director yet |
| Platform | `render` (+ 5 backends), `input`, `audio`, `client`, `distributor` | `RhiDeviceFactory`; Metal, Vulkan, DX12, OpenGL, and stub backends, one compiled in per binary; pad and audio-output backends, SDL3 on desktop; the SDL3 `DesktopGameClient`, which owns the pads and the sound. Distributor packages are stubs, not yet wired into the build |
| Editor | `project`, `shell` | Project open and create against `.simplish/project.json` through a native dialog, a recent-projects list, and the editor shell: title bar, menu bar with dropdown menus, tool toolbar, a pan-and-zoom dimetric viewport, an asset strip whose models — OBJ, or rigged glTF that plays its animation clips — drag into the world as depth-tested 3D meshes, and click-to-select with a properties panel that moves and turns what is selected, and gives a prop a behavior that makes it an actor in the playtest |
...
**What does not exist yet:** every engine package marked *to write* in §6, and the game past players and actors — no weapons, no loadouts, no director. Actors attack and players take damage, go down and are revived; nothing a player does hurts anything yet. The editor's Play button now drives the simulation tick with the game's world; the isometric renderer and the projectile system — the other systems this project is actually about — are M1 and M2 work that starts from the foundation above.
