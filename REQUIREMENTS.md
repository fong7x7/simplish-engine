...
| Game | `content`, `player`, `combat`, `actors`, `world`, `fx` | Players spawned from a `GameSetup` as the character each picked, taking its speed and health, moved by their stick on the deterministic tick, kept out of the level's solid props, and hashed. Actors — enemies and NPCs — that perceive the players, run a behavior's state machine ([ADR-009](docs/decisions/ADR-009-actor-behavior-state-machines.md)), plan paths round the props or walk a player's flow field, steer, turn, patrol routes laid out in the editor, take each other on, and attack — bites, volleys, hazard pools, blasts — through an effects buffer the damage phase applies, from twelve built-in behaviors or the project's own ([actors.md](docs/game/actors.md)) — 2,000 of them inside Engine §7's AI budget, measured by `scripts/perf-gate.sh`. Every shot fired, shot landed and blast is cued for presentation, and plays a muzzle flash, sparks or a spray, or a fireball, lighting the meshes around it ([fx.md](docs/engine/fx.md)), and is heard — the nearest four of each kind a tick, from where it happened ([audio.md](docs/engine/audio.md)). Enemy archetypes are data for the director to spawn. A determinism test and a replay round-trip over the whole world. No weapons, damage or director yet |
