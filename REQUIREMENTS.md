...
|---|---|---|
| Engine | `math`, `core`, `image`, `render`, `gui`, `client`, `render-mesh`, `render-fx`, `render-sprite`, `animation`, `gltf`, `sim`, `input`, `audio`, `physics`, `spatial` | Math (deterministic sine and cosine among it), allocators, logging, event bus, engine init, expression evaluator, plugin host, audit system, PCG32 and the fixed-step clock; the deterministic tick, SoA entity slots with generational handles, per-subsystem tick hashing, and replay record/encode/verify ([simulation.md](docs/engine/simulation.md)); input actions from keys and pads through a remappable binding scheme, quantised into the `PlayerInput` a tick runs on ([input.md](docs/engine/input.md)); an upright cylinder resolved out of axis-aligned boxes; a navigation grid built from those boxes, line of sight across it and deterministic A* over it ([spatial.md](docs/engine/spatial.md)); the abstract RHI interface (29 headers); a retained-mode GUI with layout, widgets, docking, theming, FreeType text, and a markdown renderer; `GameClient` / `RenderedGameClient`; rigged glTF models posed by animation clips, crossfaded between them, and skinned on the GPU, for the few characters that are not sprites ([animation.md](docs/engine/animation.md)); bursts of particles drawn over the scene's depth, and flashes that light meshes ([fx.md](docs/engine/fx.md)); sprite sheets cut into frames and the upright quads one frame is drawn on, cut out of their background by the mesh pass's alpha test ([sprites.md](docs/engine/sprites.md)); WAV and Ogg clips, synthesised stand-ins, and a software mixer — voice stealing by priority, buses, music ducking, panning around a listener — on the output's thread ([audio.md](docs/engine/audio.md)) |
