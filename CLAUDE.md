...
| A game's own menus and HUD: `content/ui/*.ui.json`, `game/ui`, showing screens from the logic, `ui_action` input, bound flags, the screens' `theme.json`, `set_ui_screen` / `set_ui_theme` / `render_ui_screen` / `press_ui` | [docs/game/ui.md](docs/game/ui.md) and [ADR-012](docs/decisions/ADR-012-game-menus-as-data.md) — screens are data built from the engine's GUI widgets; the logic sets values, never widgets; a choice is player input |
...
| Painted water: the ripple simulation, the water surface and its shader, water depth, flow and viscosity, how water is lit, refraction and reflections, the scene copy it is drawn from, wakes and splashes, the water fidelity setting | [docs/engine/water.md](docs/engine/water.md) — presentation only: stepped on the frame's clock, never read by a tick |
...
| GUI look: colours, spacing, radii, button variants, hover/pressed/disabled/selected states, theme files | [docs/engine/gui/technical/theming.md](docs/engine/gui/technical/theming.md) — widgets read `ctx.activeTheme()`; state looks blend on change |
| GUI widgets and overlays: which widgets exist (checkbox, toggle, radio, tabs, progress, number field, virtual list, table, tree view), menus, tooltips, modal dialogs, toasts | [docs/engine/gui/technical/widgets.md](docs/engine/gui/technical/widgets.md) §0 and [overlays.md](docs/engine/gui/technical/overlays.md) — floating things live in the tree's overlay layer |
| GUI motion and inspection: easing, springs, things arriving and leaving, layout glides, reduced motion, the layout overlay, `get_widgets`, the widget gallery | [docs/engine/gui/technical/motion.md](docs/engine/gui/technical/motion.md) and [inspecting.md](docs/engine/gui/technical/inspecting.md) — animate drawing, never layout; land animations before a still capture |
| GUI layout: flex rows and columns, padding, margins, gaps, sizing, grow/shrink, overlays, a widget's natural size | [docs/engine/gui/technical/layout-engine.md](docs/engine/gui/technical/layout-engine.md) — CSS flexbox in border-box pixels, with recipes; set `tree_layout` and let `computeLayout` place it, never hand-computed rects |
...
- `src/editor/` — the desktop editor (`project`, `build`, `shell`, `agent`,
  `deploy`).
...
a layer of its own over the ground — depth, colour, opacity, flow and
viscosity per cell — its ripples simulated on the frame's clock, going round props and
carried downstream, and drawn from a copy of the scene as a lit surface
through which the terrain shows, bent, with reflections, wet shores and
lingering foam, Flat, Low or High — [docs/engine/water.md](docs/engine/water.md)), `animation` and `gltf` (rigged models
...
tool painting its ground, and the Build menu: a project's game logic
